...
module.exports = function(sequelize, DataTypes){
  ...
  var Schedule = sequelize.define("Schedule", {
    ...
    // Batch counterpart of user.promise_schedule_I_obey(): fetches schedules
    // for all given users in a single query and populates each user's
    // "cached_schedule", so subsequent promise_schedule_I_obey() calls do not
    // hit the database. Users that already have a cached schedule are skipped.
    Schedule.promise_to_cache_schedules_for = function(args){
        var users = args.users || [];

        var users_to_load = users.filter(function(user){
          return ! user.cached_schedule;
        });
        ...
        var user_ids    = Array.from(new Set( users_to_load.map(function(u){ return u.id }) )),
            company_ids = Array.from(new Set( users_to_load.map(function(u){ return u.companyId }) ));
        ...
        return Schedule
          ...
          .then(function(schedules){
            ...
            schedules.forEach(function(schedule){
              if (schedule.is_user_specific()) {
                user_specific[schedule.user_id] = schedule;
              } else {
                company_wide[schedule.company_id] = schedule;
              }
            });

            // Companies without a schedule in DB get a default one; build it
            // once per company as it carries no user specific state
            var default_by_company = {};

            users_to_load.forEach(function(user){
              var schedule = user_specific[user.id] || company_wide[user.companyId];

              if ( ! schedule ) {
                if ( ! default_by_company[user.companyId] ) {
                  default_by_company[user.companyId] = Schedule.build({
                    company_id : user.companyId,
                  });
                }
                schedule = default_by_company[user.companyId];
              }
