...
function CalendarMonth(day, args){
    ...
    this._leaves        = {};
    this._bank_holidays = {};
    this._working_day_overrides = {};
    this.locale         = (args && args.locale) ? args.locale : 'en';
    ...
    if (args && args.working_day_overrides){
        var workingDayOverrides = {};
        args.working_day_overrides.forEach(function(day){
            workingDayOverrides[moment.utc(day.date).format(self.default_date_format())] = true;
        });
        self._working_day_overrides = workingDayOverrides;
    }
...
CalendarMonth.prototype.is_weekend = function(day){
  if (this._working_day_overrides[
    this.get_base_date().date(day).format(this.default_date_format())
  ]) {
    return false;
  }
...
CalendarMonth.prototype.is_calendar_weekend = function(day){
  const weekDay = this.get_base_date().date(day).day();
...
// Считается ли отпуск на этот день в календарных днях (тогда он покрывает выходные)
CalendarMonth.prototype.is_calendar_days_leave = function(day){
  var self = this;
  var ids = [ this.get_morning_leave_type_id(day), this.get_afternoon_leave_type_id(day) ];
  return ids.some(function(id){
    var lt = id && self._leave_types_map[ id ];
    if ( ! lt ) { return false; }
    return typeof lt.is_calendar_days === 'function'
      ? lt.is_calendar_days()
      : lt.deduction_unit === 'calendar_days';
  });
};
...
CalendarMonth.prototype.as_for_template = function(){
    ...
    for( var i=1; i<=self.how_many_days(); i++){
        var day = {
          val       : i,
          moment    : self.get_base_date().date(i),
          leave_obj : self.get_leave_obj(i),
        };

        if ( self.is_calendar_weekend(i) ) {
            day.is_calendar_weekend = true;
        }
        ...
        // Раскраска отпуска: на рабочие дни — всегда; на выходные — только если
        // тип отсутствия считается в календарных днях (отпуск покрывает и выходные).
        if ( ! day.is_bank_holiday && ( ! day.is_weekend || self.is_calendar_days_leave(i) ) ) {
    ...
    return {
        month : self.get_base_date().locale(self.locale).format('MMMM'),
