...
const filterLeaves = ({startDate, endDate}) => (leave) => {
  ...
  // Closed intervals overlap when the leave starts no later than the report
  // end and finishes no earlier than the report start. Checking only whether
  // either leave endpoint falls inside the report misses a leave that fully
  // spans a one-day connector run.
  return sd.isSameOrBefore(endDate, 'day') && ed.isSameOrAfter(startDate, 'day');
};
