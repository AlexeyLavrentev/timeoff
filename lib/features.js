...
const getLicensedFeatureSet = () => {
  ...
  const requestedFeatures = parseList(process.env.TIMEOFF_FEATURES || config.get('licensed_features'));
  const licensedFeatures = parseList(payload.features);
  const values = requestedFeatures.concat(licensedFeatures);
...
const isEnabled = featureName => {
  ...
  if (getLicensedFeatureSet().has(name)) {
    return true;
  }
