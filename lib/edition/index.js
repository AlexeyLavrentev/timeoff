  ...
  try {
    ...
    if (result && typeof result.catch === 'function') {
      result.catch(function(error) {
        console.error('Leave event dispatcher error (suppressed): ' + (error && error.stack || error));
      });
    }
  } catch (error) {
    console.error('Leave event dispatcher error (suppressed): ' + (error && error.stack || error));
  ...
  try {
    ...
    if (result && typeof result.then === 'function') {
      return result.catch(function(error) {
        console.error('Supervised department provider error (suppressed): ' + (error && error.stack || error));
        return [];
      });
    }
