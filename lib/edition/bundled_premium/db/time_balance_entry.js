...
module.exports = function(sequelize, DataTypes) {
  const TimeBalanceEntry = sequelize.define("TimeBalanceEntry", {
    ...
    TimeBalanceEntry.associate = function(models) {
    ...
    TimeBalanceEntry.entry_type_time_off = () => 'time_off';

    TimeBalanceEntry.entry_type_worked_extra = () => 'worked_extra';

    TimeBalanceEntry.status_new = () => 1;

    TimeBalanceEntry.status_approved = () => 2;

    TimeBalanceEntry.status_rejected = () => 3;

    TimeBalanceEntry.status_canceled = () => 4;

    TimeBalanceEntry.prototype.is_new = function() {
        return this.status === TimeBalanceEntry.status_new();
      };

    TimeBalanceEntry.prototype.is_approved = function() {
        return this.status === TimeBalanceEntry.status_approved();
      };

    TimeBalanceEntry.prototype.signed_hours = function() {
        return this.entry_type === TimeBalanceEntry.entry_type_time_off()
          ? -1 * this.hours
          : this.hours;
      };

    TimeBalanceEntry.prototype.is_expired = function() {
    ...
    TimeBalanceEntry.prototype.promise_to_approve = function(args) {
    ...
    TimeBalanceEntry.prototype.promise_to_reject = function(args) {
        if (!args || !args.by_user) {
