...
function getApplicationDomain() {
  return String(branding.get().applicationDomain || '').replace(/\/+$/, '');
}
...
async function findAnyUserByEmail(email) {
  return model.User.findOne({ where: { email: normalizeEmail(email) } });
}
...
async function getEnabledSsoCompanyById(companyId) {
  ...
  const company = await model.Company.findOne({ where: { id: companyId } });
...
async function renderSamlMetadata(req, res) {
  const company = await model.Company.findOne({ where : { id : req.params.companyId } });
