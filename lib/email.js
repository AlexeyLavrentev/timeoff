...
const
  ...
  emailTemplatePaths = require('./email_template_paths'),
...
function resolve_email_template(filename) {
  const templateName = filename + '.hbs';
  const matchedPath = emailTemplatePaths.get()
    .map(templatePath => path.join(templatePath, templateName))
    .find(candidate => fs.existsSync(candidate));

  return matchedPath || path.join(emailTemplatePaths.defaultEmailTemplatePath, templateName);
}
...
Email.prototype.promise_rendered_email_template = function(args){
  ...
  .then(function(){
    return handlebars.render(
      resolve_email_template(filename),
      context
    );
  })
...
Email.prototype.get_send_email = function(){
  ...
  if (! config.get("send_emails") || ! config.get("email_transporter") ) {
    return function(){
      if (process.env.SILENCE_PRETEND_EMAILS !== 'true') {
...
Email.prototype.promise_leave_request_emails = function(args){
  ...
  const promise_email_to_requestor = ({comments, requesterAllowance}) => self.promise_rendered_email_template({
    ...
    context : {
      ...
      approver  : leave.get('approver'),
      requester : leave.get('user'),
      user      : leave.get('user'),  // Use requester instead of approver
      requesterAllowance,
    }
  })
  ...
  return Promise.all([
    ...
    .then(([comments, requesterAllowance]) => bluebird.join(
      ...
      bluebird.resolve(leave.get('user').promise_supervisors())
...
Email.prototype.promise_upcoming_leave_start_reminder_email = function(args){
  var self = this;
  var send_mail = self.get_send_email();
  ...
  var isEmployeeReminder = notification_type === 'leave_start_reminder_employee';
  var template_name = isEmployeeReminder
    ? 'leave_start_reminder_to_employee'
    : 'leave_start_reminder_to_supervisor';
  ...
  .then(function(email_obj){
    ...
    .then(function(){
      return recipient.record_email_addressed_to_me(email_obj);
    });
  });
};
