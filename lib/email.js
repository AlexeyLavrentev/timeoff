...
Email.prototype.promise_upcoming_leave_start_reminder_email = function(args){
  ...
  const isEmployeeReminder = args.notification_type === 'leave_start_reminder_employee';
  const template_name = isEmployeeReminder
  ...
  .then(email_obj => send_mail({
    ...
    .then(() => recipient.record_email_addressed_to_me(email_obj))
  );
};

Email.prototype.promise_time_balance_request_emails = function(args){
  const self = this;
  ...
  const send_to = ({recipient, template_name}) => {
    if (!recipient) {
      return bluebird.resolve();
    }
  ...
  return bluebird.join(
    send_to({recipient : supervisor, template_name : 'time_balance_request_to_supervisor'}),
    send_to({recipient : requester, template_name : 'time_balance_request_to_requestor'}),
    () => bluebird.resolve()
  );
};

Email.prototype.promise_time_balance_decision_email = function(args){
  const self = this;
  const entry = args.entry;
  const requester = entry.get('user');
  ...
  .then(email_obj => send_mail({
    ...
    .then(() => requester.record_email_addressed_to_me(email_obj))
  );
};
