...
const
  ...
  {getUsersWithLeaves} = require('../model/Report'),
  {
    getReplacementPlan,
    parseStatuses,
  } = require('../model/replacement_planner');
  ...
  router.get(
    ...
    (req, res) => {
      const startDate = validator.isDate(req.query['start_date'])
        ? moment.utc(req.query['start_date'])
        : (
          validator.isDate(req.query['date'])
      ...
      const leaveStatuses = parseStatuses({
        rawStatuses : req.query['leave_statuses'],
      });
      ...
      .then(data => res.json(data))
      .catch(error => {
        console.log(`An error occured when trying to access /report/replacements: ${error} at ${error.stack}`);
        res.json({ error: `${error}` });
      });
    }
  );
