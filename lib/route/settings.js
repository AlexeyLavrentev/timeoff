...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .then(function(foundCompany){
      ...
      ldapParameters = get_and_validate_ldap_auth_configuration({
        req : req,
      });
      ssoParameters = get_and_validate_sso_auth_configuration({
        req : req,
      });
      ...
      if (ssoParameters.sso_auth_enabled) {
...
function get_and_validate_sso_auth_configuration(args) {
  var req = args.req;
  var sso_auth_enabled = validator.toBoolean(req.body['sso_auth_enabled']);
  var sso_auth_provider = validator.trim(req.body['sso_auth_provider'] || 'oidc').toLowerCase();
  var login_alias = validator.trim(req.body['sso_login_alias']).toLowerCase();
  var issuer_url = validator.trim(req.body['sso_issuer_url']);
  var client_id = validator.trim(req.body['sso_client_id']);
  var client_secret = validator.trim(req.body['sso_client_secret']);
  var scope = validator.trim(req.body['sso_scope'] || 'openid profile email');
  var email_claim = validator.trim(req.body['sso_email_claim'] || 'email');
  var require_verified_email = validator.toBoolean(req.body['sso_require_verified_email']);
  var entry_point = validator.trim(req.body['sso_entry_point']);
  var idp_cert = validator.trim(req.body['sso_idp_cert']);
  var identifier_format = validator.trim(req.body['sso_identifier_format']);
  var email_attribute = validator.trim(req.body['sso_email_attribute'] || 'email');
  var email_domains = validator.trim(req.body['sso_email_domains']);
  var auto_create_users = validator.toBoolean(req.body['sso_auto_create_users']);
  var sp_entity_id = validator.trim(req.body['sso_sp_entity_id']);
