...
function get_and_validate_leave_type(args) {
  ...
  if ( ! validator.isNumeric(minimum_consecutive_days) ){
    throw_user_error( 'Minimum consecutive days for '+item_name+' should be a valide number' );

  } else if ( minimum_consecutive_days < 0) {
    throw_user_error( 'Minimum consecutive days for '+item_name+' should be positive number or 0' );
  }
