...
router.get('/company/replacements/', (req, res) => {
  const model = req.app.get('db_model');
  const departmentFilter = validator.isInt(`${req.query.department || ''}`)
  ...
  req.user
    ...
    .then(c => {
      company = c;
      users = (company.users || []).sort((a, b) => sorter(a.lastname, b.lastname));

      if (departmentFilter) {
        users = users.filter(user => Number(user.DepartmentId) === departmentFilter);
      }

      return company.getDepartments();
    })
    .then(items => {
      departments = (items || []).sort((a, b) => sorter(a.name, b.name));

      return model.UserReplacement.findAll({
        where : {
    ...
    .then(rules => {
      if (departmentFilter) {
        rules = rules.filter(rule => rule.user && Number(rule.user.DepartmentId) === departmentFilter);
      }

      rules = rules.sort((a, b) => {
        const aName = a.user ? a.user.full_name() : '';
        const bName = b.user ? b.user.full_name() : '';
        const byName = sorter(aName, bName);
      ...
      res.render('settings_company_replacements', {
        company,
        users,
        departments,
        departmentFilter,
    ...
    .catch(error => {
      console.error(
        `Failed to fetch replacement rules for user ${req.user.id}: ${error} at ${error.stack}`
      );

      req.session.flash_error(req.t('settings.messages.saveFailed', {
        reason: req.t('settings.messages.contactSupport'),
      }));
...
router.post('/company/replacements/', (req, res) => {
  const model = req.app.get('db_model');

  const userId = validator.trim(req.body.user_id || '');
  const replacementUserId = validator.trim(req.body.replacement_user_id || '');
  const priorityRaw = validator.trim(req.body.priority || '1');

  if (
    !validator.isInt(userId) ||
    !validator.isInt(replacementUserId) ||
    Number(userId) < 1 ||
    Number(replacementUserId) < 1
  ) {
    req.session.flash_error(req.t('settings.messages.replacementUserOrEmployeeUnknown'));
    return res.redirect_with_session('/settings/company/replacements/');
  }

  if (String(userId) === String(replacementUserId)) {
    req.session.flash_error(req.t('settings.messages.replacementSelf'));
    return res.redirect_with_session('/settings/company/replacements/');
  }

  if (!validator.isInt(priorityRaw) || Number(priorityRaw) < 1) {
    req.session.flash_error(req.t('settings.messages.replacementPriorityInvalid'));
    return res.redirect_with_session('/settings/company/replacements/');
  }
  ...
  req.user
    ...
    .then(company => {
      const usersMap = (company.users || []).reduce((memo, user) => {
        memo[String(user.id)] = true;
        return memo;
      }, {});

      if (!usersMap[userId] || !usersMap[replacementUserId]) {
        req.session.flash_error(req.t('settings.messages.replacementUserOrEmployeeUnknown'));
        throw new Error('Replacement rule points to user outside active company users scope');
      }

      return model.UserReplacement.create({
        companyId: company.id,
        userId: Number(userId),
        replacementUserId: Number(replacementUserId),
        priority,
      });
    })
    .then(() => {
      req.session.flash_message(req.t('settings.messages.replacementRuleCreated'));
      return res.redirect_with_session('/settings/company/replacements/');
    })
    .catch(error => {
      console.error(
        `Failed to create replacement rule by user ${req.user.id}: ${error} at ${error.stack}`
      );

      if (!req.session.flash_has_errors()) {
        req.session.flash_error(req.t('settings.messages.replacementRuleCreateFailed'));
      }
      return res.redirect_with_session('/settings/company/replacements/');
    });
});

router.post('/company/replacements/delete/:rule_id/', (req, res) => {
  const model = req.app.get('db_model');
  const ruleId = validator.trim(req.params.rule_id || '');

  if (!validator.isInt(ruleId) || Number(ruleId) < 1) {
    req.session.flash_error(req.t('settings.messages.replacementRuleNotFound'));
    return res.redirect_with_session('/settings/company/replacements/');
  }

  req.user
    .getCompany()
    .then(company => model.UserReplacement.findOne({
      where : {
        id: Number(ruleId),
        companyId: company.id,
      },
    ...
    .then(rule => {
      if (!rule) {
        req.session.flash_error(req.t('settings.messages.replacementRuleNotFound'));
        throw new Error(`Replacement rule ${ruleId} was not found`);
      }

      return rule.destroy();
    })
    .then(() => {
      req.session.flash_message(req.t('settings.messages.replacementRuleDeleted'));
      return res.redirect_with_session('/settings/company/replacements/');
    })
    .catch(error => {
      console.error(
        `Failed to remove replacement rule ${ruleId} by user ${req.user.id}: ${error} at ${error.stack}`
      );

      if (!req.session.flash_has_errors()) {
        req.session.flash_error(req.t('settings.messages.replacementRuleDeleteFailed'));
      }
      return res.redirect_with_session('/settings/company/replacements/');
    });
});
...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .catch(function(error){
      ...
      req.session.flash_error(req.t('settings.messages.ldapUpdateFailed', {
