...
const { parseReportFilters } = require('../model/report_filter');
...
router.get('/', (req, res) => {
  res.render('report/index', {
    title: req.t('reportsPage.title'),
  });
});

router.get('/allowancebytime/', (req, res) => {
  const today = req.user.company.get_today();
  const reportFilters = parseReportFilters({
    query: req.query,
  ...
  if (reportFilters.error) {
    req.session.flash_error(req.t('reportsPage.invalidFilters'));
    return res.redirect_with_session('./');
  }

  const start_date = reportFilters.startDate;
  const end_date = reportFilters.endDate;
  ...
  const current_deparment_id = reportFilters.departmentId;
    ...
    return Promise
      .try(() => req.query['as-csv']
        ...
        : res.render('report/allowancebytime', {
          title               : req.t('reportsAllowanceByTime.title'),
...
const extractParametersForLeavesReport = ({req, actingUser}) => {
  const today = actingUser.company.get_today();
  return parseReportFilters({
    query: req.query,
    defaultStartDate: today.clone().startOf('month'),
    defaultEndDate: today.clone().endOf('month'),
    moment,
    validator,
...
const renderLeavesReportAsCsv = async ({res, company, startDate, endDate, leaves}) => {
  ...
  const csvString = await csv.stringify( content );
...
router.get('/leaves/', async (req, res) => {
  ...
  const reportFilters = extractParametersForLeavesReport({req, actingUser});
  ...
  const { startDate, endDate, departmentId, leaveTypeId } = reportFilters;
  ...
  if (renderAsCsv) {
    ...
    res.render('report/leaves', {
      title: req.t('reportsLeaves.title'),
      leaves,
      departmentId,
      leaveTypeId,
      sortBy,
