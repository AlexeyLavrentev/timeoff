...
router.get('/summary/:userId/', (req, res) => {
  ...
  let flow = req.app.get('db_model').User.scope('withDepartments').findOne({where:{id:userId}});
  ...
  flow = flow.then(user => {
    ...
    return requestor.promise_supervised_users()
      .then(users => {
  ...
  flow = flow.then(({user, isDetailed}) => {
    ...
    return Promise.all([user.promise_allowance(), user.promise_supervisors()])
  ...
  flow.catch(error => {
    console.log(error);
    res
      .status(error.tom_error ? 404 : 500)
      .send(req.t('errors.userDetailsFailed'));
  });
