...
router.post('/add/', function(req, res){
  ...
  .then(function(){
    if ( req.session.flash_has_errors() ) {
      ...
      req.session.flash_message(req.t('users.messages.created'));
  ...
  .catch(function(error){
    ...
    if (error && error.code === 'LICENSE_SEAT_LIMIT_EXCEEDED') {
      req.session.flash_error(req.t('users.messages.seatLimitReached', {
        count: error.maxActiveUsers,
      }));
    } else if ( error && error.tom_error) {
      req.session.flash_error( Exception.extract_user_error_message(error) );
    }

    req.session.flash_error(req.t('users.messages.createFailed'));
...
router.post('/import/', function(req, res){
  ...
  // formidable v3 exposes parse() as a promise of [fields, files]; every field
  // and uploaded file is provided as an array of values.
  Promise.resolve(form.parse(req))
    .then(([fields, files]) => {
      const pickFirst = value => Array.isArray(value) ? value[0] : value;
      const submittedCsrf = pickFirst(fields && fields._csrf);
      const uploaded = pickFirst(files && files.users_import);

      if (!authSecurity.tokensMatch(req.session && req.session.csrf_token, submittedCsrf)) {
        Exception.throw_user_error({
          user_error: req.t('login.messages.invalidCsrfToken'),
          system_error: 'Invalid CSRF token on users CSV import',
        });
      }
      ...
      return fs.readFileAsync(uploaded.filepath || uploaded.path, "utf8");
    })
    .then(csv_data_string => csv.parse(csv_data_string,{trim:true}))
    .then(parsed_data => {
    ...
    .then(action_result => {
      if ( action_result.users.length > 0 ) {
        req.session.flash_message(req.t('users.messages.importSuccess', {
          emails: action_result.users.map(user => user.email).sort().join(', ')
        }));
      }
      if (action_result.errors.length > 0) {
        action_result.errors.forEach(err => req.session.flash_error(
          req.t('users.messages.importUserFailed', { email: err.email, reason: err.error })
        ));
      }
    ...
    .catch(function(error){
      ...
      req.session.flash_error(req.t('users.messages.importFailed', {
    ...
    return model.User.find_by_email(new_user_attributes.email)
      .then(function(user){
        ...
        if (user && user.companyId !== employee.companyId) {
          var error_msg = req.t('users.messages.emailExistsOtherAccount');
...
var ensure_email_is_not_used_elsewhere = function(args){
  ...
  return model.User
    ...
    .then(function(user){
      ...
      if (user) {
        req.session.flash_error(req.t('users.messages.emailAlreadyInUse'));
  ...
  if (
    ...
    return model.User
      ...
      .then(function(number_of_admins_to_be_left){
        ...
        req.session.flash_error(req.t('users.messages.lastAdmin'));
...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(){
    ...
    return employee
      ...
      .then(function(){
        return teamViewCache.bumpCompanyVersion(employee.companyId);
      })

      .then(function(){
        req.session.flash_message(req.t('users.messages.detailsUpdated', {
  ...
  .catch(function(error){
    ...
    if (error && error.code === 'LICENSE_SEAT_LIMIT_EXCEEDED') {
      ...
      req.session.flash_error(req.t('users.messages.saveFailed'));
    }
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(() => teamViewCache.bumpCompanyVersion(req.user.companyId))
    .then(() => {
      req.session.flash_message(req.t('users.messages.recordsRemoved'));
      return res.redirect_with_session('../..');
    })
    .catch(error => {
      console.error(
...
router.all('/search/', function(req, res){
  ...
  if ( ! validator.isEmail( email )) {
    req.session.flash_error(req.t('users.messages.invalidSearchEmail', { email }));
    return res.json([]);
  }
  ...
  promise_result.then(function(company){
    ...
    res.status(500).json({error: 'user_search_failed'});
  });
});
...
router.get('/', function(req, res) {
    ...
    if (!employeeGroupsEnabled || !validator.isNumeric(group_id)) {
      group_id = undefined;
    }
    ...
    if (group_id) {
      group_include.where = { id : group_id };
    }

    if (employeeGroupsEnabled) {
      user_include.splice(1, 0, group_include);
    }
    ...
    .then(function(company){
      return company.getBank_holidays()
        ...
        .then(function(groups){
          company.groups = groups.sort((a,b) => sorter(a.name, b.name));
...
function users_list_as_csv(args) {
  ...
  return res.send( csv.stringify( content ) );
}
...
function get_and_validate_user_parameters(args) {
    ...
    if (!name) {
        req.session.flash_error(req.t('users.validation.nameRequired', {
          itemName: item_name
        }));
    }

    if (!lastname) {
        req.session.flash_error(req.t('users.validation.lastNameRequired', {
          itemName: item_name
        }));
    }

    if (!validator.isEmail(email)) {
        req.session.flash_error(req.t('users.validation.emailInvalid', {
          itemName: item_name
        }));
    }

    if (!validator.isNumeric(department_id)) {
        req.session.flash_error(req.t('users.validation.departmentInvalid', {
          itemName: item_name
        }));
    }

    if (adjustment && ! validator.isFloat(adjustment) ) {
      req.session.flash_error(req.t('users.validation.adjustmentInvalid', {
        itemName: item_name
      }));
    } else if (adjustment && ! ( adjustment % 1 === 0 || Math.abs( adjustment % 1 ) === 0.5 )) {
      req.session.flash_error(req.t('users.validation.adjustmentInvalidHalf', {
        itemName: item_name
      }));
    }
    ...
    if (!validator.isDate(start_date)) {
      req.session.flash_error(req.t('users.validation.startDateInvalid', {
        itemName: item_name
      }));
    }
    ...
    if (end_date ){
      ...
      if ( ! validator.isDate(end_date)) {
        req.session.flash_error(req.t('users.validation.endDateInvalid', {
          itemName: item_name
        }));
      }
    }
    ...
    if (
        ...
        req.session.flash_error(req.t('users.validation.endDateBeforeStart', {
          itemName: item_name
        }));
    }

    if (password && password !== password_confirm) {
      req.session.flash_error(req.t('users.validation.passwordMismatch'));
    }

    if (require_password && ! password) {
      req.session.flash_error(req.t('users.validation.passwordRequired'));
    }
