...
function find_department_with_same_name(Department, company_id, name) {
  const normalized_name = String(name || '').trim().toLocaleLowerCase();

  return Department.findAll({
    where : {companyId : company_id},
  })
  .then(departments => departments.find(
    department => String(department.name || '').trim().toLocaleLowerCase() === normalized_name
  ));
}
...
function get_and_validate_department(args) {
  ...
  const work_calendar_field = no_suffix ? 'work_calendar_id' : 'work_calendar_id__'+index;
  const reminder_field = no_suffix ? 'notify_leave_start_reminder' : 'notify_leave_start_reminder__'+index;
  const reminder_to_employee_field = no_suffix
    ? 'notify_leave_start_reminder_to_employee'
    : 'notify_leave_start_reminder_to_employee__'+index;
  const work_calendars_enabled = features.isEnabled('work_calendars');
  const leave_start_reminders_enabled = features.isEnabled('leave_start_reminders');
  ...
  let
    ...
    notify_leave_start_reminder = validator.toBoolean(
      req.body[reminder_field]
    ),
    notify_leave_start_reminder_to_employee = validator.toBoolean(
      req.body[reminder_to_employee_field]
    ),
    work_calendar_id = validator.trim(
      req.body[work_calendar_field] || ''
    );

  // Validate provided parameters
  //
  if (!name) {
    req.session.flash_error(req.t('departments.messages.nameRequired'));
  }

  // New allowance should be from range of (0;50]
  if (!validator.isFloat(allowance)) {
    req.session.flash_error(req.t('departments.messages.allowanceNumeric', {
      department: department_name
    }));
  } else if (!((0 <= allowance) && (allowance <= 50))) {
    req.session.flash_error(req.t('departments.messages.allowanceRange', {
      department: department_name
    }));
  }

  if (work_calendar_id && !work_calendars_enabled) {
    req.session.flash_error(req.t('features.messages.disabled'));
  } else if (work_calendar_id && !_.contains(
    _.map(company.work_calendars || [], calendar => String(calendar.id)),
    String(work_calendar_id)
  )) {
    req.session.flash_error(req.t('departments.messages.workCalendarUnknown', {
      department: department_name
    }));
  }

  if (
    !leave_start_reminders_enabled
    && (
      Object.prototype.hasOwnProperty.call(req.body, reminder_field)
      || Object.prototype.hasOwnProperty.call(req.body, reminder_to_employee_field)
    )
  ) {
    req.session.flash_error(req.t('features.messages.disabled'));
  }
  ...
  if (!validator.isNumeric( boss_id ) ) {
    req.session.flash_error(req.t('departments.messages.bossNumeric', {
      department: department_name
    }));
  } else if ( ! _.contains(
    ...
    req.session.flash_error(req.t('departments.messages.bossUnknown', {
      department: department_name
    }));
  }
  ...
  return {
    ...
    notify_leave_start_reminder : leave_start_reminders_enabled
      ? notify_leave_start_reminder
      : !!department.notify_leave_start_reminder,
    notify_leave_start_reminder_to_employee : leave_start_reminders_enabled
      ? notify_leave_start_reminder_to_employee
      : !!department.notify_leave_start_reminder_to_employee,
    WorkCalendarId          : work_calendars_enabled
      ? (work_calendar_id || null)
      : (department.WorkCalendarId || null),
    name                    : name,
  };
}
...
router.get('/departments/', function(req, res){
  ...
  req.user.getCompany({
    scope : ['with_active_users', 'order_by_active_users', 'with_work_calendars'],
  })
  ...
  .then(function(departments){
    res.render('departments_overview', {
      title             : req.t('titles.departmentsSettings'),
...
router.post('/departments/', function(req, res){
  ...
  req.user.getCompany({
    scope : ['with_active_users', 'with_work_calendars'],
  })
  .then(company => {
    ...
    return find_department_with_same_name(
      model.Department,
      company.id,
      attributes.name
    )
    .then(existing => {
      if (existing) {
        req.session.flash_error(req.t('departments.messages.duplicateName'));
        return null;
      }

      return model.Department.create(attributes);
    })
    .then(created => created && teamViewCache.bumpCompanyVersion(company.id));
  })

  .then(() => {
    if ( ! req.session.flash_has_errors() ) {
      req.session.flash_message(req.t('departments.messages.saved'));
    }
  ...
  .catch(error => {
    ...
    req.session.flash_error(req.t('departments.messages.createFailed'));
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  if (!validator.isInt(department_id)) {
    ...
    req.session.flash_error(req.t('departments.messages.removeWrongParams'));
  ...
  .then(function(departments){
    ...
    if (department_to_remove.users.length > 0){
      req.session.flash_error(req.t('departments.messages.removeHasUsers', {
        name: department_to_remove.name,
        count: department_to_remove.users.length
      }));
    ...
    return department_to_remove.destroy()
      .then(() => teamViewCache.bumpCompanyVersion(req.user.companyId));
  })
  .then(function(){
    req.session.flash_message(req.t('departments.messages.removed'));
    return res.redirect_with_session('/settings/departments/');
  })
...
function promise_to_extract_company_and_department(req, only_active = true) {
  ...
  return Promise.try(function(){
    ...
    if ( ! validator.isInt(department_id)) {
      const error = new Error(
        'User '+req.user.id+' tried to open department refered by non-int ID '+department_id
      );
    ...
    if (only_active) {
      ...
      return req.user.getCompany({
        scope : ['with_all_users', 'with_work_calendars'],
      });
    }
  })
  .then(function(c){
  ...
  .then(function(departments){
    ...
    if ( ! department ) {
      const error = new Error('Non existing department ID provided');
      error.statusCode = 404;
      throw error;
    }
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(result){
    ...
    res.render('department_details', {
      title      : req.t('titles.departmentDetails'),
      department : department,
      company    : company,
      allowance_options : generate_all_department_allowances(),
    });
  ...
  .catch(function(error){
    ...
    req.session.flash_error(req.t('departments.messages.detailsFailed'));
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    if (req.body.remove_supervisor_id) {
      ...
      .then(function(){
        req.session.flash_message(req.t('departments.messages.supervisorRemoved', {
      ...
      .then(function(){
        req.session.flash_message(req.t('departments.messages.supervisorsAdded', {
          department: department.name
        }));
        return Promise.resolve(1);
      });
    ...
    .then(function(){
      req.session.flash_message(req.t('departments.messages.updated', {
        department: department.name
      }));
      return Promise.resolve(1);
    });
  ...
  .then(function(){
    return teamViewCache.bumpCompanyVersion(company.id);
  })
  ...
  .catch(function(error){
    ...
    if (!req.session.flash_has_errors()) {
      req.session.flash_error(req.t('departments.messages.updateFailed'));
    }

    return res.redirect_with_session(department ? '.' : '../../');
  });
});
...
function promise_to_remove_supervisor(args) {
  ...
  return department.constructor.sequelize.models.DepartmentSupervisor.destroy({
    where : {
...
function promise_to_update_supervisors(args) {
  ...
  var link_model = department.constructor.sequelize.models.DepartmentSupervisor;

  return department.constructor.sequelize.transaction(transaction =>
    link_model.destroy({
      where : {
        department_id : department.id,
      },
...
function promise_to_update_department(args) {
  ...
  .then(existing => {
    if (existing && String(existing.id) !== String(department.id)) {
      req.session.flash_error(req.t('departments.messages.duplicateName'));
      throw new Error('Department name is already in use');
    }
...
router.get('/departments/available-supervisors/:department_id/', function(req, res){
  ...
  .catch(function(error){
    ...
    res
      ...
      .send(req.t('errors.requestFailed'));
  });
});
