...
function get_and_validate_department(args) {
  ...
  let
    ...
    max_critical_overlap = validator.trim(req.body[no_suffix ? 'max_critical_overlap' : 'max_critical_overlap__'+index]),
  ...
  if (!validator.isInt(max_critical_overlap, { min: 1, max: 50 })) {
    req.session.flash_error(req.t('departments.messages.criticalOverlapRange', {
      department: department_name
    }));
  }
  ...
  return {
    ...
    max_critical_overlap    : max_critical_overlap,
    name                    : name,
  };
}
...
function promise_to_update_department(args) {
  ...
  const critical_user_ids_raw = req.body.critical_user_ids;
  const critical_user_ids = Array.isArray(critical_user_ids_raw)
    ? critical_user_ids_raw
    : critical_user_ids_raw
      ? [critical_user_ids_raw]
      : [];

  const allowed_critical_ids = company.users
    .filter(user => user.DepartmentId === department.id)
    .map(user => String(user.id));

  const normalized_critical_ids = critical_user_ids
    .map(id => String(id))
    .filter(id => allowed_critical_ids.indexOf(id) !== -1);

  return department.updateAttributes(attributes)
    .then(() => Promise.map(
      company.users.filter(user => user.DepartmentId === department.id),
      user => {
        const should_be_critical = normalized_critical_ids.indexOf(String(user.id)) !== -1;
        if (user.is_critical === should_be_critical) {
          return Promise.resolve();
        }
        user.is_critical = should_be_critical;
        return user.save();
      },
      { concurrency: 5 }
    ));
