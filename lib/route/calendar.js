...
router.get('/teamview/', async (req, res) => {
  ...
  const compact_mode = getCompactModeParameter(req);
  ...
  const team_view = months_count > 1
    ? new TeamView({ user, start_date, end_date })
    : new TeamView({ user, base_date: normalized_base_date });
...
const getCompactModeParameter = (req) => {
  let compactMode = !! req.query['compact_mode'];

  if (req.query['save_compact_mode']) {
    req.session.teamViewCompactMode = compactMode;
  }

  if (req.query['compact_mode'] === undefined) {
    compactMode = req.session.teamViewCompactMode;
  }
