...
const buildTeamViewMonths = ({
  ...
  while (cursor.isSameOrBefore(lastMonth, 'month')) {
    ...
    const monthUsers = users_and_leaves.map(userNode => ({
      ...userNode,
      days: userNode.days.filter(day => {
        const dayMoment = moment.utc(day.moment);
        return dayMoment.isSameOrAfter(monthStart, 'day') && dayMoment.isSameOrBefore(monthEnd, 'day');
      }),
    }));
