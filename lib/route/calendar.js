...
const buildTeamViewMonths = ({ users_and_leaves, start_date, end_date, grouped_mode }) => {
  const months = [];
  ...
  while (cursor.isSameOrBefore(lastMonth, 'month')) {
    ...
    const monthData = {
      ...
      end_date: monthEnd,
      users_and_leaves: monthUsers,
    };

    if (grouped_mode) {
      monthData.users_and_leaves_by_departments = groupUsersOnTeamViewByDepartments(monthUsers);
    }
