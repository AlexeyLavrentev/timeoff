...
router.all(/.*/, features.requireFeature('employee_groups'));
...
router.get('/groups/', function(req, res){
  ...
  .then(company => Promise.join(
    company.getGroups({
      ...
      order : [[model.sequelize.col('Group.' + model.Group.default_order_field()), 'ASC']],
    }),
...
router.post('/groups/', function(req, res){
  ...
  const max_critical_overlap = validator.trim(req.body['max_critical_overlap'] || '1');
  const is_hr_group = Boolean(req.body['is_hr_group']);
  ...
  if (!validator.isInt(max_critical_overlap, { min: 1, max: 50 })) {
    req.session.flash_error(req.t('groups.messages.criticalOverlapRange', {
      group: name || req.t('groups.details'),
    }));
    return res.redirect_with_session('/settings/groups/');
  }
...
router.get('/groups/edit/:group_id/', function(req, res){
  ...
  .then(company => Promise.join(
    model.Group.findOne({
      ...
      include : [{ model : model.User, as : 'users', through: { attributes: ['is_critical'] } }],
    }),
    Promise.resolve(company),
  ...
  .then(({group, company}) => {
    ...
    const group_critical_map = group.users.reduce((acc, user) => {
      if (user.UserGroup && user.UserGroup.is_critical) {
        acc[user.id] = true;
      }
      return acc;
    }, {});
    ...
    res.render('group_details', {
      title : req.t('groups.details'),
      group : group,
      company : company,
      group_user_ids : group_user_ids,
      group_user_map : group_user_map,
      group_critical_map : group_critical_map,
    });
  })
  .catch(error => {
    ...
    return req.user.getCompany({ scope : ['with_active_users'] })
      .then(company => {
        ...
        const normalized_critical_ids = critical_user_ids
          .map(id => String(id))
          .filter(id => normalized_member_ids.indexOf(id) !== -1);
        ...
        return group.update({
          ...
          .then(() => model.UserGroup.update(
            { is_critical: false },
            { where: { groupId: group.id } }
          ))
          .then(() => normalized_critical_ids.length === 0
            ? Promise.resolve()
            : model.UserGroup.update(
              { is_critical: true },
              { where: { groupId: group.id, userId: normalized_critical_ids } }
            )
          )
          .then(() => group);
      });
