...
describe('Settings accessibility and routing contracts', function() {
  it('gives every leave-type editor control a contextual accessible name', function() {
    ...
    expect(view).to.match(/<button type="button"[^>]*data-toggle="dropdown"[^>]*aria-label="{{t "generalSettings\.leaveTypeColorAction"}}: {{name}}"/);
    ...
    expect(view).to.match(/<button[^>]*class="[^"]*leavetype-remove-btn[^"]*"[^>]*aria-label="{{t "common\.remove"}}: {{name}}"/);
  });
