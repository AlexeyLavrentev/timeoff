...
1. В `config/app.json` параметр `application_domain` должен указывать на внешний HTTPS-адрес приложения.
2. Если приложение стоит за reverse proxy, наружный URL должен совпадать с тем, который видит пользователь и Keycloak.
3. Для multi-tenant сценария желательно заполнить `SSO email domains`, чтобы приложение могло определить компанию до редиректа в IdP.
4. Если хотите, чтобы сотрудник создавался автоматически при первом успешном входе, включите `Allow automatic user provisioning`.
5. Если auto provisioning выключен, пользователь должен уже существовать в приложении и иметь тот же email, что и в Keycloak.
...
- `SSO email domains`: один или несколько корпоративных доменов, например `example.com`
- `Allow automatic user provisioning`: включите, если хотите создавать сотрудника автоматически при первом входе
- `Require verified email from OIDC provider`: включайте только если в Keycloak реально выставляется `email_verified=true`

### Как это работает

1. Пользователь нажимает `Continue with SSO` на странице логина и переходит на отдельную SSO-страницу.
2. Если в системе только одна SSO-компания, приложение может сразу отправить пользователя в IdP.
3. Иначе пользователь вводит рабочий email, а приложение определяет компанию по существующему пользователю или по `SSO email domains`.
4. Если для компании активен OIDC, пользователь перенаправляется в Keycloak.
5. После callback приложение либо находит локального пользователя, либо создаёт нового сотрудника автоматически, если включён auto provisioning.
...
1. Откройте страницу `/login/` и нажмите `Continue with SSO`.
2. Если редирект сразу не произошёл, введите рабочий email на SSO-странице.
3. Убедитесь, что происходит redirect в Keycloak.
4. После успешного логина пользователь должен вернуться в приложение уже в авторизованной сессии.
5. Если `Allow automatic user provisioning` включён, новый сотрудник должен появиться в приложении автоматически.
